...
export default function TalentForm() {
    ...
    if (isSuccess) {
        return (
            ...
            >
                ...
                <p className="text-slate-300 font-sans font-medium text-xl md:text-2xl mb-8">
    ...
    return (
        <motion.form
            ...
            className="glass-card p-10 md:p-16 rounded-[2.5rem] max-w-3xl mx-auto space-y-8"
        >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                    <label className="text-base md:text-lg font-semibold text-slate-300 ml-2">Full Name</label>
                    <input
                        required
                        name="name"
                        type="text"
                        placeholder="John Doe"
                        className="w-full bg-slate-900/50 border border-slate-800 rounded-2xl px-6 py-4 text-lg text-white focus:outline-none focus:border-brand-blue transition-colors font-sans"
                    />
                </div>
                <div className="space-y-2">
                    <label className="text-base md:text-lg font-semibold text-slate-300 ml-2">Email Address</label>
                    <input
                        required
                        name="email"
                        type="email"
            ...
            <div className="space-y-2">
                <label className="text-base md:text-lg font-semibold text-slate-300 ml-2">LinkedIn Profile URL</label>
                <input
                    required
                    name="linkedin"
                    type="url"
            ...
            <div className="space-y-2">
                <label className="text-base md:text-lg font-semibold text-slate-300 ml-2">Area of Expertise</label>
                <select
                    required
                    name="expertise"
                    defaultValue=""
                    className="w-full bg-slate-900/50 border border-slate-800 rounded-2xl px-6 py-4 text-lg text-white focus:outline-none focus:border-brand-blue transition-colors font-sans appearance-none"
                >
            ...
            <div className="space-y-2">
                <label className="text-base md:text-lg font-semibold text-slate-300 ml-2">Resume / CV (PDF)</label>
                <div
                    onDragOver={onDragOver}
                    onDragLeave={onDragLeave}
                    onDrop={onDrop}
                    className={`relative border-2 border-dashed rounded-3xl p-12 transition-all flex flex-col items-center justify-center text-center cursor-pointer ${isDragging ? 'border-brand-blue bg-brand-blue/5' : 'border-slate-800 hover:border-slate-700'
                    ...
                    <AnimatePresence mode="wait">
                        {!file ? (
                            ...
                            >
                                ...
                                <p className="text-slate-500 text-sm uppercase tracking-wider">PDF files only (max 10MB)</p>
                            </motion.div>
                        ) : (
                            <motion.div
                                key="file-info"
            ...
            <button
                ...
                className="w-full bg-brand-blue hover:bg-brand-blue-light disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-5 text-lg md:text-xl rounded-2xl transition-all shadow-[0_0_20px_rgba(37,99,235,0.2)] flex items-center justify-center gap-3 active:scale-[0.98]"
            >
                {isSubmitting ? (
                    <>
